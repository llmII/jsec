...
Janet cfun_hkdf(int32_t argc, Janet *argv) {
    ...
    if (length <= 0 || length > 255 * 64) {
        crypto_panic_param("output length must be 1-%d, got %d", 255 * 64,
                           length);
    }
...
Janet cfun_pbkdf2(int32_t argc, Janet *argv) {
    ...
    if (length <= 0 || length > 1024)
        crypto_panic_param("output length must be 1-1024, got %d", length);
    ...
    if (!PKCS5_PBKDF2_HMAC((const char *)password.bytes, password.len,
                           salt.bytes, salt.len, iterations, md, length,
                           out)) {
