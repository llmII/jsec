...
static const JanetReg cfuns[] = {
    ...
    {"generate-csr", cfun_generate_csr,
     ...
     "  :san - array of Subject Alt Names (e.g. [\"DNS:example.com\" "
     "\"IP:1.2.3.4\"])\n"
    ...
    {"cms-sign", cfun_cms_sign,
     ...
     "Options:\n"
     "  :detached - if true, creates detached signature (content not "
     "included)\n"
     "Returns DER-encoded CMS structure."},
    {"cms-verify", cfun_cms_verify,
     "(jsec/crypto/cms-verify cms-der &opt trusted-certs-pem "
     "detached-data)\n\n"
    ...
    {"cms-encrypt", cfun_cms_encrypt,
     ...
     "Options:\n"
     "  :cipher - encryption cipher (:aes-128-cbc, :aes-192-cbc, "
     ":aes-256-cbc, :3des)\n"
     "            Default is :aes-256-cbc\n"
     "Returns DER-encoded CMS structure."},
    {"cms-decrypt", cfun_cms_decrypt,
     "(jsec/crypto/cms-decrypt cms-der cert-pem key-pem)\n\n"
     "Decrypt CMS enveloped data using recipient's certificate and private "
     "key.\n"
    ...
    {"cms-certs-only", cfun_cms_certs_only,
     ...
     "certs can be a PEM string (possibly containing multiple certs) or "
    ...
    {"export-key", cfun_export_key,
     ...
     "Options:\n"
     "  :password - password for encryption (if not provided, key is "
     "unencrypted)\n"
     "  :cipher - encryption cipher (:aes-256-cbc, :aes-128-cbc, "
     ":des-ede3-cbc)\n"
     "            Default is :aes-256-cbc\n"
     "Returns PEM format key."},
    {"key-info", cfun_key_info,
     "(jsec/crypto/key-info key-pem)\n\n"
     "Get metadata about a key without needing the password for encrypted "
     "keys.\n"
     "Returns table with:\n"
