...
extern void dtls_client_async_callback(JanetFiber *fiber, JanetAsyncEvent event);
...
Janet cfun_dtls_connect(int32_t argc, Janet *argv) {
    ...
    JanetStream *transport = janet_stream(fd, JANET_STREAM_READABLE | JANET_STREAM_WRITABLE, NULL);
    ...
    client->stream.handle = JANET_HANDLE_NONE;  /* We don't own the handle directly */
