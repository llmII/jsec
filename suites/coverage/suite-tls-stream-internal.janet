...
(assay/def-suite :name "TLS Stream Internal API Coverage"
                 :timeout 30
                 ...
                 (assay/def-test "set-ocsp-response on connection"
                 ...
                 (assay/def-test "accept directly on listener"
                                 :harness [:certs {:setup (fn [_ _] (cert/generate-self-signed-cert {:common-name "localhost"}))}]
                                 (let [server (net/listen "127.0.0.1" "0")
                                       [_ port] (net/localname server)
                                       done (ev/chan 1)
                                     ...
                                     (with [conn (net/connect "127.0.0.1" (string port))]
                                       (with [tls-conn (tls-stream/wrap conn "localhost" {:verify false})]
                                         ...
                                         (let [echo (:read tls-conn 100)]
                                           (assay/assert (= (string echo) "echo-test") "Direct accept works"))))

                                     (ev/take done))))

                 (assay/def-test "accept-loop handles multiple clients"
                                 :harness [:certs {:setup (fn [_ _] (cert/generate-self-signed-cert {:common-name "localhost"}))}]
                                     ...
                                     (for i 0 3
                                       (with [conn (net/connect "127.0.0.1" (string port))]
                                         (with [tls-conn (tls-stream/wrap conn "localhost" {:verify false})]
                                           ...
                                           (let [echo (:read tls-conn 100)]
                                             (assay/assert (= (string echo) (string "msg-" i))
                                                           (string "Accept loop client " i " works"))))))

                                     # Verify all handled
                                     (for i 0 3
                                       (assay/assert (= :ok (ev/take handled)) (string "Handler " i " completed"))))))

                 (assay/def-test "direct stream functions read/write/shutdown"
                                 :harness [:certs {:setup (fn [_ _] (cert/generate-self-signed-cert {:common-name "localhost"}))}]
                                 (let [server (net/listen "127.0.0.1" "0")
                                       [_ port] (net/localname server)
                                       done (ev/chan 1)]
                                     ...
                                     (with [conn (net/connect "127.0.0.1" (string port))]
                                       (with [tls-conn (tls-stream/wrap conn "localhost" {:verify false})]
                                         ...
                                         (let [echo (tls-stream/read tls-conn 100)]
                                           (assay/assert (= (string echo) "direct-test") "Direct functions work"))
                                         (tls-stream/shutdown tls-conn)))

                                     (ev/take done))))

                 (assay/def-test "chunk function reads exact bytes"
                                 :harness [:certs {:setup (fn [_ _] (cert/generate-self-signed-cert {:common-name "localhost"}))}]
                                 (let [server (net/listen "127.0.0.1" "0")
                                       [_ port] (net/localname server)
                                       done (ev/chan 1)
                                   ...
                                   (defer (:close server)
                                     ...
                                     (with [conn (net/connect "127.0.0.1" (string port))]
                                       (with [tls-conn (tls-stream/wrap conn "localhost" {:verify false})]
                                         ...
                                         (assay/assert (= (length buf) chunk-size)
                                                       (string "Chunk read exactly " chunk-size " bytes"))))

                                     (ev/take done))))

                 (assay/def-test "session functions get-session and session-reused?"
                                 :harness [:certs {:setup (fn [_ _] (cert/generate-self-signed-cert {:common-name "localhost"}))}]
                                 (let [server (net/listen "127.0.0.1" "0")
                                       [_ port] (net/localname server)
                                       done (ev/chan 1)]
                                     ...
                                     (with [conn (net/connect "127.0.0.1" (string port))]
                                       (with [tls-conn (tls-stream/wrap conn "localhost" {:verify false})]
                                         ...
                                         (let [session (tls-stream/get-session tls-conn)]
                                           (assay/assert session "get-session returns session data"))
                 ...
                 (assay/def-test "connection info functions version/cipher/bits"
                                 :harness [:certs {:setup (fn [_ _] (cert/generate-self-signed-cert {:common-name "localhost"}))}]
                                 (let [server (net/listen "127.0.0.1" "0")
                                       [_ port] (net/localname server)
                                       done (ev/chan 1)]
                                     ...
                                     (with [conn (net/connect "127.0.0.1" (string port))]
                                       (with [tls-conn (tls-stream/wrap conn "localhost" {:verify false})]
                                         ...
                                         (let [version (tls-stream/get-version tls-conn)]
                                           (assay/assert (string? version) "get-version returns string")
                                   ...
                                   (defer (:close server)
                                     ...
                                     (with [conn (net/connect "127.0.0.1" (string port))]
                                       (with [tls-conn (tls-stream/wrap conn "localhost" {:context ctx})]
                                         (let [data (tls-stream/read tls-conn 100)]
                                           (assay/assert (= (string data) "trusted") "Trust cert on context works"))))

                                     (ev/take done))))

                 (assay/def-test "listen directly creates raw listener"
                                 (let [listener (tls-stream/listen "127.0.0.1" "0" {})]
                                   (defer (:close listener)
                                     (assay/assert listener "Created listener")
                                     # Listener should be a core stream (not TLSStream)
                                     (let [[_ port] (net/localname listener)]
                                       (assay/assert (> port 0) "Listener has valid port")))))

                 (assay/def-test "renegotiate on TLS 1.2"
                                 :harness [:certs {:setup (fn [_ _] (cert/generate-self-signed-cert {:common-name "localhost"}))}]
                                 (let [server (net/listen "127.0.0.1" "0")
                                       [_ port] (net/localname server)
                                       done (ev/chan 1)]
                                     ...
                                     (with [conn (net/connect "127.0.0.1" (string port))]
                                       (with [tls-conn (tls-stream/wrap conn "localhost"
                                         ...
                                         (try
                                           ...
                                           ([err] nil))
                                         (let [data (tls-stream/read tls-conn 100)]
                                           (assay/assert (= (string data) "renegotiated") "Renegotiate test completed"))))

                                     (ev/take done))))

                 (assay/def-test "key-update on TLS 1.3"
                                 :harness [:certs {:setup (fn [_ _] (cert/generate-self-signed-cert {:common-name "localhost"}))}]
                                 (let [server (net/listen "127.0.0.1" "0")
                                       [_ port] (net/localname server)
                                       done (ev/chan 1)]
                                     ...
                                     (with [conn (net/connect "127.0.0.1" (string port))]
                                       (with [tls-conn (tls-stream/wrap conn "localhost"
                                         ...
                                         (try
                                           ...
                                           ([err] nil))
                                         (let [data (tls-stream/read tls-conn 100)]
                                           (assay/assert (= (string data) "key-updated") "Key-update test completed"))))

                                     (ev/take done))))

                 (assay/def-test "handshake timing measurement"
                                 :harness [:certs {:setup (fn [_ _] (cert/generate-self-signed-cert {:common-name "localhost"}))}]
                                 (let [server (net/listen "127.0.0.1" "0")
                                       [_ port] (net/localname server)
                                       done (ev/chan 1)]
                                     ...
                                     (with [conn (net/connect "127.0.0.1" (string port))]
                                       (with [tls-conn (tls-stream/wrap conn "localhost"
                                         ...
                                         (let [hs-time (tls-stream/get-handshake-time tls-conn)]
                                           (assay/assert (or (nil? hs-time) (number? hs-time))
                                                         "get-handshake-time returns number or nil")
                                           (when hs-time
                                             (assay/assert (> hs-time 0) "handshake time is positive")))))
