...
(assay/def-suite :name "DTLS Stream Suite"

                 # ===========================================================================
                 # Non-Matrix Tests (Basic Functionality)
                 # ===========================================================================

                 (assay/def-test "Address Type"
                                 (let [addr (tls/address "192.168.1.1" 443)]
                                   (assay/assert (tls/address? addr) "Created address")
                                   (assay/assert (= (tls/address-host addr) "192.168.1.1") "Host matches")
                                   (assay/assert (= (tls/address-port addr) 443) "Port matches")))

                 (assay/def-test "IPv6 Address"
                                 (let [addr (tls/address "::1" 8443)]
                                   (assay/assert (tls/address? addr) "Created IPv6 address")
                                   (assay/assert (= (tls/address-host addr) "::1") "IPv6 host matches")
                                   (assay/assert (= (tls/address-port addr) 8443) "IPv6 port matches")))

                 (assay/def-test "Timeout"
                                 (let [certs (generate-temp-certs {:common-name "127.0.0.1"})
                                   ...
                                   (:close server)
                                   (assay/assert result "Server recv timed out as expected")))

                 (assay/def-test "Invalid port connection fails"
                                 :expected-fail "Connection to invalid port should fail"
                                 (ev/with-deadline 2
                                   (def client (tls/connect "127.0.0.1" 1 {:datagram true :verify false}))
                                   (:write client "test")))

                 (assay/def-test "Bad cert verification fails"
                 ...
                 (assay/def-test "Server localname"
                                 (let [certs (generate-temp-certs {:common-name "127.0.0.1"})
                                       server (tls/listen "127.0.0.1" 0
                                                          {:datagram true :cert (certs :cert) :key (certs :key)})
                                       [host port] (:localname server)]
                                   (:close server)
                                   (assay/assert (= host "127.0.0.1") "Host is localhost")
                                   (assay/assert (> port 0) "Port was assigned")))

                 (assay/def-test "DTLS verify-hostname option"
                                 (let [certs (generate-temp-certs {:common-name "localhost"})
                                   ...
                                   (let [client (tls/connect "127.0.0.1" port
                                     ...
                                     (assay/assert (= response "hello") "DTLS verify-hostname: received data")
                                     (:close client true))
                 ...
                 (assay/def-test "DTLS Config Matrix"
                                 :type :matrix
                                 ...
                                 (assay/def-test "basic echo"
                                                 (let [server-opts (make-dtls-server-opts dtls-version cipher-group certs)
                                                   ...
                                                   (assay/assert (= (string reply) "Pong") "Got Pong")
                                                   (:close client true)
                                 ...
                                 (assay/def-test "large message"
                                                 (let [server-opts (make-dtls-server-opts dtls-version cipher-group certs)
                                                   ...
                                                   (assay/assert (= (length reply) msg-size) "Large message echoed correctly")))
                                 ...
                                 (assay/def-test "connection info"
                                                 (let [server-opts (make-dtls-server-opts dtls-version cipher-group certs)
                                                   ...
                                                   (assay/assert (struct? info) "Connection info is a struct")
                                                   (when (info :cipher)
                                                     (assay/assert (string? (info :cipher)) "Cipher is a string"))
                                 ...
                                 # -------------------------------------------------------------------------
                                 (assay/def-test "ALPN negotiation"
                                                 (let [server-opts (merge (make-dtls-server-opts dtls-version cipher-group certs)
                                                   ...
                                                   (assay/assert (= (string reply) "alpn-ok") "ALPN negotiation completed")))

                                 # -------------------------------------------------------------------------
                                 # multiple simultaneous clients - Concurrency test
                                 # -------------------------------------------------------------------------
                                 (assay/def-test "multiple simultaneous clients"
                                                 (let [server-opts (make-dtls-server-opts dtls-version cipher-group certs)
                                                       client-opts (make-dtls-client-opts verify-mode cipher-group certs)
                                                       num-clients 3
                                                       server (tls/listen "127.0.0.1" 0 server-opts)
                                 ...
                                 # -------------------------------------------------------------------------
                                 (assay/def-test "close with force flag"
                                                 (let [server-opts (make-dtls-server-opts dtls-version cipher-group certs)
                                                       client-opts (make-dtls-client-opts verify-mode cipher-group certs)
                                                       server (tls/listen "127.0.0.1" 0 server-opts)
                                                       [_ port] (:localname server)
