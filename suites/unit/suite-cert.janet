...
(def-suite :name "Cert Suite"

  (def-test "Generate Self-Signed Cert"
    (let [certs (cert/generate-self-signed-cert {:common-name "test-cert"})]
      (assert (string? (certs :cert)) "Cert is string")
      (assert (string? (certs :key)) "Key is string")
      (assert (> (length (certs :cert)) 0) "Cert not empty")
      (assert (> (length (certs :key)) 0) "Key not empty")))

  (def-test "Buffer Cert Loading"
            ...
            (let [s1 (tls/connect "127.0.0.1" port {:hostname "127.0.0.1"
              ...
              (defer (:close s1)
                ...
                (let [response (:read s1 1024)]
                  (assert (= (string response) "OK") "Received OK")))))
          (error port)))))

  (def-test "Malformed cert data"
    (assert-error (tls-stream/new-context {:cert "NOT A VALID CERTIFICATE"
                                           :key "NOT A VALID KEY"})))

  (def-test "Invalid options type"
    (assert-error (cert/generate-self-signed-cert "not-a-table")))

  (def-test "Valid minimal options"
    (let [certs (cert/generate-self-signed-cert)]
      (assert (certs :cert) "Has cert")
      (assert (certs :key) "Has key"))))
